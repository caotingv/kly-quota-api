...
class Query(object):
    ...
    def query_cli_flavor(self):
        ...
        if self.edu_info and self.bus_info:
            ...
            self.vcpus = math.ceil((self.edu_info['flavor']['vcpu'] * self.edu_info['number'] + self.bus_info['flavor']['vcpu'] * self.bus_info['number'])/3)
        elif self.edu_info:
    ...
    def query_scene_sql(self,weight,cpu_manufacturer):
        ...
        try:
            ...
            all_scene_motherboard_sql = self.db.query(Motherboard).filter(Motherboard.scene_weight == weight, Motherboard.cpu_model.startswith(cpu_manufacturer))
...
class CPUQuery(Query):
    ...
    def query_reality_concurrency_level(self,sql,cli_concurrency_level):
        ...
        with_concurrency_sql = sql.filter(Motherboard.concurrency_level == cli_concurrency_level).first()
        if  with_concurrency_sql is None or self.vcpus >= with_concurrency_sql.cpu_threads:
    ...
    def query_cpu_threads_data(self, sql, concurrency_level):
        ...
        all_query_data = sql.filter(Motherboard.concurrency_level <= concurrency_level).all()
        if all_query_data == []:
            raise
    ...
    def _build_server_info_dict(server_num,data):
        return {
            "number": server_num,
            "info": data
        }
    ...
    def query_server(self):
        ...
        intel_server_info = self._server_cpu_query('intel',scene_weight,cli_concurrency_level)
    ...
    def _server_cpu_query(self, cpu_manufacturer, scene_weight, cli_concurrency_level):
        ...
        reality_concurrency_level = self.query_reality_concurrency_level(all_sql_query, cli_concurrency_level)
        server_info_list = self.query_cpu_threads_data(all_sql_query, reality_concurrency_level)
